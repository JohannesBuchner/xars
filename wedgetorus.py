...
def test_random_wedge():
	...
	for i in range(20):
		while True:
			d = dict(Theta_low=scipy.random.uniform(0,pi,2),
				NH=10**(scipy.random.uniform(20,25,2)-22),
				r_inner = scipy.random.uniform(0,1,2),
				verbose = True)
			d['Theta_high'] = scipy.random.uniform(d['Theta_low'],pi,2)
			d['r_outer'] = scipy.random.uniform(d['r_inner'],1,2)
	...
	for i in range(50):
		while True:
			d = dict(Theta_low=scipy.random.uniform(0,pi,2),
				...
				r_inner = scipy.random.uniform(0,1,2),
				verbose = False)
			d['Theta_high'] = scipy.random.uniform(d['Theta_low'] + 1 / 180*pi,pi,2)
			d['r_outer'] = scipy.random.uniform(d['r_inner'] + 0.01,1,2)
		...
		if plotting:
			...
			plt.close()
		N = 100000
		(xi, yi, zi) = [scipy.random.uniform(-0.3, 0.3, N) for i in range(3)]
		dist = 10**scipy.random.uniform(-3, 3, N)
		beta  = scipy.random.uniform(0, pi, N)
		alpha = scipy.random.uniform(0, 2*pi, N)
		#yi *= 0
		#alpha *= 0
		if plotting:
			geo.viz()
