...
import numpy
import scipy
from numpy import cos, log10, logical_and, pi, sin, tan
...
class WedgeTorusGeometry:
    ...
    def is_valid(Theta_low, Theta_high, r_inner, r_outer, NH):
        n = len(NH)
        if len(Theta_low) != n:
            return False, ['Theta_low length wrong', Theta_low]
        if len(Theta_high) != n:
            return False, ['Theta_high length wrong', Theta_high]
        if len(r_inner) != n:
            return False, ['r_inner length wrong', r_inner]
        if len(r_outer) != n:
            return False, ['r_outer length wrong', r_outer]
        for i in range(n):
            if not (Theta_low[i] >= 0):
                return False, ['Theta_low[%d] negative' % i, Theta_low[i]]
            if not (Theta_low[i] < Theta_high[i]):
                return False, ['component %d, Theta_low above Theta_high' % i, Theta_low[i], Theta_high[i]]
            if not (r_inner[i] < r_outer[i]):
                return False, ['component %d, r_inner above r_outer' % i, r_inner[i], r_outer[i]]
            # looking through to source radially, the column density should be NH
            for j in range(i):
    ...
    def compute_next_point(self, location, direction):
        ...
        if self.verbose:
            print('ray from', (xi, yi, zi), 'to', (xv, yv, zv))
        ...
        for i in range(self.n):
            ...
            if self.verbose:
                print('already inside %d' % i, already_inside)
            ...
            for inter in d1i, d2i, d1o, d2o:
                ...
                if self.verbose:
                    print('   sphere surface intersection with %d:' % i, inter, relevant)
            ...
            for inter in d1l, d2l, d1h, d2h:
                rf, thetaf, phif = to_spherical((xi + inter * xv, yi + inter * yv, zi + inter * zv))
                if self.verbose:
                    print('   potential cone surface intersection with %d:' % i, inter, (rf, thetaf, phif))
                ...
                inter[~relevant] = numpy.nan
                if self.verbose:
                    print('   cone surface intersection with %d:' % i, inter, relevant)
                intersections.append((i * numpy.ones_like(inter, dtype=numpy.uint), inter))
        ...
        if self.verbose:
            print('handling intersections; distance to go:', dist)
        for i in range(len(intersections)):
            ...
            distance = di - last_d
            if self.verbose:
                print('within %d for %f [%f .. %f]', (oi[point_exit], distance[point_exit], last_d[point_exit], di[point_exit]))

            if self.plot:
                plt.plot(
                    [0.4 + 0.3 * (xi[point_exit] + last_d[point_exit] * xv[point_exit]),
            ...
            if self.verbose:
                print('terminating %d, at' % termination.sum(), d_interaction)
            if numpy.any(termination):
                ...
                if self.plot:
                    plt.plot(
                        0.4 + 0.3 * xf[termination], 0.5 + 0.3 * zf[termination],
                ...
                if self.verbose:
                    print('termination update:', terminate_inside_scene)

            dist[gothrough] -= nh[gothrough]
            if self.verbose:
                print('distance left:', dist[gothrough])
            last_d[point_exit] = numpy.nan
            last_o[point_exit] = numpy.nan

        # we leave the scene
        if self.verbose:
            print('termination inside:', terminate_inside_scene)
